...
def decrypt_secret(key, data):
    ...
    try:
        ...
        encrypted_data = json.loads(
            fernet.decrypt(bytes(data, encoding="utf-8")).decode("utf-8")
        )
    except:
...
def request_user_information(execute_info, secret_info):
    ...
    headers = {
        "x-api-key": secret_info.cide_api_key,
        "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
    }
    params = {
        "client_id": secret_info.cide_client_id,
    ...
    if status_code != 200:
        raise RequestError(
            f"メール抽出APIのリクエストでエラーが発生しました。\n・リクエストエラーメッセージ{response_json['message']}",
            status_code,
        )
...
def generate_csv(secret_info, limit_row, gz_files, csv_file_path):
    ...
    for file_num, file in enumerate(gz_files):
        ...
        for loop_num in range(loop_count):
            ...
            user_info_df = tmp_user_info_df.loc[start_row:end_row].compute(
                num_workers=multiprocessing.cpu_count()
            )
            decode_username_df = transform_df(secret_info, user_info_df)
            try:
                decode_username_df.to_csv(
                    f"{csv_file_path}\{file_num}_{loop_num}.csv", index=False
                )
            except:
...
def transform_df(secret_info, df):
    ...
    try:
        df["userName"] = df["userName"].apply(
            lambda x: username_decode(secret_info, x).decode("utf-8")
        )
    except:
...
if __name__ == "__main__":
    ...
    secret_info_path = os.path.join(
        current_dir.parent.parent, f"enc_secret_file_{env}.text"
    )
